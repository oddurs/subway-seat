# Subway Seat for Vimium

Harvest-gold link hints, a walnut Vomnibar with denim URLs and orange matches, and the find bar and help dialog to match.

[Vimium](https://vimium.github.io) · [Previews and copy buttons](https://oddurs.github.io/subway-seat/ports/vimium/) · Needs Vimium 2.0+

## Files

| Flavor | File | Where it goes |
|---|---|---|
| Subway Seat | [`subway-seat.css`](subway-seat.css) | paste into Vimium Options › CSS for Vimium UI |
| Subway Seat Tunnel | [`subway-seat-tunnel.css`](subway-seat-tunnel.css) | paste into Vimium Options › CSS for Vimium UI |
| Subway Seat Enamel | [`subway-seat-enamel.css`](subway-seat-enamel.css) | paste into Vimium Options › CSS for Vimium UI |
| London Moquette | [`london-moquette.css`](london-moquette.css) | paste into Vimium Options › CSS for Vimium UI |
| London Deep Level | [`london-deep-level.css`](london-deep-level.css) | paste into Vimium Options › CSS for Vimium UI |
| London Portland | [`london-portland.css`](london-portland.css) | paste into Vimium Options › CSS for Vimium UI |
| Paris Guimard | [`paris-guimard.css`](paris-guimard.css) | paste into Vimium Options › CSS for Vimium UI |
| Paris Catacombes | [`paris-catacombes.css`](paris-catacombes.css) | paste into Vimium Options › CSS for Vimium UI |
| Paris Carrelage | [`paris-carrelage.css`](paris-carrelage.css) | paste into Vimium Options › CSS for Vimium UI |
| All three | [`subway-seat-auto.css`](subway-seat-auto.css) | paste into Vimium Options › CSS for Vimium UI |

## Turn it on

**Subway Seat**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of subway-seat.css and click Save
```

**Subway Seat Tunnel**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of subway-seat-tunnel.css and click Save
```

**Subway Seat Enamel**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of subway-seat-enamel.css and click Save
```

**London Moquette**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of london-moquette.css and click Save
```

**London Deep Level**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of london-deep-level.css and click Save
```

**London Portland**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of london-portland.css and click Save
```

**Paris Guimard**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of paris-guimard.css and click Save
```

**Paris Catacombes**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of paris-catacombes.css and click Save
```

**Paris Carrelage**, in Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of paris-carrelage.css and click Save
```

## Follow light and dark

In Vimium Options › Show advanced options › CSS for Vimium UI:

```text
Paste the contents of subway-seat-auto.css and click Save
# Enamel while the system is light, Subway Seat (Walnut) while it's dark
```

Generated by `build.py` from `palette.py` (v0.3.0). Edit the port in `ports/`, not these files.
