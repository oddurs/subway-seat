...
& {
    ...
    $colors = @{
        ...
        Command                = "${esc}[38;2;147;98;0m"                     # yellow
        ...
        Keyword                = "${esc}[38;2;173;78;0m"                     # orange
        Comment                = "${esc}[3;38;2;140;114;84m"                 # overlay1, italic
        Error                  = "${esc}[38;2;188;64;49m"                    # red_hi
        Emphasis               = "${esc}[1;38;2;147;98;0m"                   # yellow, bold: search matches
        Selection              = "${esc}[1;38;2;42;29;19;48;2;202;180;142m"  # text_hi on the selection ground
        ContinuationPrompt     = "${esc}[38;2;140;114;84m"                   # overlay1
        InlinePrediction       = "${esc}[38;2;165;140;106m"                  # overlay0, like fish autosuggestions
        ListPrediction         = "${esc}[38;2;173;78;0m"                     # orange: the > marker and source
        ListPredictionSelected = "${esc}[48;2;202;180;142m"                  # surface1 ground
    ...
    if ($PSStyle) {
        $formatting = @{
            FormatAccent           = "${esc}[1;38;2;147;98;0m"
            TableHeader            = "${esc}[1;38;2;147;98;0m"
            CustomTableHeaderLabel = "${esc}[1;3;38;2;147;98;0m"
            ErrorAccent            = "${esc}[1;38;2;173;78;0m"
            Error                  = "${esc}[1;38;2;188;64;49m"
            Warning                = "${esc}[1;38;2;147;98;0m"
            Verbose                = "${esc}[38;2;63;100;128m"
            Debug                  = "${esc}[38;2;62;113;87m"
            FeedbackName           = "${esc}[38;2;173;78;0m"
            FeedbackText           = "${esc}[38;2;84;64;47m"
            FeedbackAction         = "${esc}[38;2;147;98;0m"
        }
        foreach ($key in $formatting.Keys) {
            if ($PSStyle.Formatting.PSObject.Properties[$key]) { $PSStyle.Formatting.$key = $formatting[$key] }
        }
        $PSStyle.Progress.Style = "${esc}[1;38;2;147;98;0m"

        if ($PSStyle.PSObject.Properties['FileInfo']) {
            $PSStyle.FileInfo.Directory = "${esc}[1;38;2;147;98;0m"
            $PSStyle.FileInfo.SymbolicLink = "${esc}[38;2;62;113;87m"
            $PSStyle.FileInfo.Executable = "${esc}[1;38;2;102;116;15m"
            foreach ($ext in '.zip', '.tgz', '.gz', '.tar', '.nupkg', '.cab', '.7z') { $PSStyle.FileInfo.Extension[$ext] = "${esc}[38;2;188;64;49m" }
