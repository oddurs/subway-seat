...
& {
    ...
    $colors = @{
        ...
        Command                = "${esc}[38;2;142;107;8m"                    # yellow
        Parameter              = "${esc}[38;2;0;110;107m"                    # sage
        String                 = "${esc}[38;2;32;127;65m"                    # green
        Operator               = "${esc}[38;2;128;75;0m"                     # orange
        Variable               = "${esc}[38;2;152;84;124m"                   # clay
        Member                 = "${esc}[38;2;61;71;62m"                     # subtext1
        Number                 = "${esc}[38;2;190;66;61m"                    # red_hi
        Type                   = "${esc}[38;2;0;110;107m"                    # sage
        Keyword                = "${esc}[38;2;128;75;0m"                     # orange
        Comment                = "${esc}[3;38;2;110;124;110m"                # overlay1, italic
        Error                  = "${esc}[38;2;190;66;61m"                    # red_hi
        Emphasis               = "${esc}[1;38;2;142;107;8m"                  # yellow, bold: search matches
        ...
        ListPrediction         = "${esc}[38;2;128;75;0m"                     # orange: the > marker and source
    ...
    if ($PSStyle) {
        $formatting = @{
            FormatAccent           = "${esc}[1;38;2;142;107;8m"
            TableHeader            = "${esc}[1;38;2;142;107;8m"
            CustomTableHeaderLabel = "${esc}[1;3;38;2;142;107;8m"
            ErrorAccent            = "${esc}[1;38;2;128;75;0m"
            Error                  = "${esc}[1;38;2;190;66;61m"
            Warning                = "${esc}[1;38;2;142;107;8m"
            Verbose                = "${esc}[38;2;39;98;156m"
            Debug                  = "${esc}[38;2;0;110;107m"
            FeedbackName           = "${esc}[38;2;128;75;0m"
            FeedbackText           = "${esc}[38;2;61;71;62m"
            FeedbackAction         = "${esc}[38;2;142;107;8m"
        }
        foreach ($key in $formatting.Keys) {
            if ($PSStyle.Formatting.PSObject.Properties[$key]) { $PSStyle.Formatting.$key = $formatting[$key] }
        }
        $PSStyle.Progress.Style = "${esc}[1;38;2;142;107;8m"

        if ($PSStyle.PSObject.Properties['FileInfo']) {
            $PSStyle.FileInfo.Directory = "${esc}[1;38;2;142;107;8m"
            $PSStyle.FileInfo.SymbolicLink = "${esc}[38;2;0;110;107m"
            $PSStyle.FileInfo.Executable = "${esc}[1;38;2;32;127;65m"
            foreach ($ext in '.zip', '.tgz', '.gz', '.tar', '.nupkg', '.cab', '.7z') { $PSStyle.FileInfo.Extension[$ext] = "${esc}[38;2;190;66;61m" }
            foreach ($ext in '.ps1', '.psd1', '.psm1', '.ps1xml') { $PSStyle.FileInfo.Extension[$ext] = "${esc}[38;2;0;110;107m" }
        }
    }
}
