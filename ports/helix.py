...
META = {
    ...
    "enable": {
        "where": "~/.config/helix/config.toml (Windows: %AppData%\\helix\\config.toml)",
        "code": 'theme = "{snake}"',
        "lang": "toml",
    },
    "auto": {
        "where": "~/.config/helix/config.toml, in place of the `theme = …` line. Needs a Helix build newer than "
        "25.07.1 (25.07.1 rejects a `[theme]` table) and a terminal that reports light or dark (mode 2031)",
        "code": '[theme]\ndark = "subway_seat"\nlight = "subway_seat_enamel"\nfallback = "subway_seat"',
        "lang": "toml",
    },
    "notes": "Copy the themes to `~/.config/helix/themes/` (Windows: `%AppData%\\helix\\themes`). The colored "
    "mode badge in the status line needs `color-modes = true` under `[editor]`. Rainbow brackets come from the "
    "theme's `rainbow` key, which Helix 25.07.1 skips with a warning in its log; builds after it read it.",
    "detect": ["hx", "helix"],
}
...
def theme(f):
    ...
    extra = {
        ...
        "selection_secondary": u["selection_inactive"],
        "bracket": u["bracket_fg"],
        "bracket_bg": u["bracket_bg"],
        "paper": u["paper"],
        "row": u["row"],
        "inlay_bg": u["inlay_bg"],
        "ink": u["ink"],
        "error_bg": u["error_bg"],
        "warning_bg": u["warning_bg"],
        "info_bg": u["info_bg"],
        "hint_bg": u["hint_bg"],
        "frameline": t["chg"],
    }
    ...
    s = {
        ...
        "variable": S("variable"),
        "variable.mutable": S("variable"),
        "variable.builtin": S("variable.builtin"),
        "variable.builtin.mutable": S("variable.builtin"),
        "variable.parameter": S("parameter"),
        "variable.parameter.mutable": S("parameter"),
        ...
        "operator": S("operator"),
        "function": S("function"),
        "function.public": S("function"),
        "function.builtin": S("function.builtin"),
        "function.method": S("function"),
        "function.method.public": S("function"),
        "function.method.private": S("function"),
        "function.macro": S("decorator"),
        ...
        "ui.cursor.match": st("bracket", "bracket_bg", ["bold"]),
        ...
        "ui.gutter.selected": st(bg="cursorline"),
        ...
        "ui.popup": st("text", "paper"),
        "ui.popup.info": st("text", "paper"),
        ...
        "ui.window": st("crust"),
        "ui.help": st("subtext1", "paper"),
        "ui.menu": st("subtext1", "paper"),
        "ui.menu.selected": st("text_hi", "row", ["bold"]),
        "ui.menu.scroll": st("overlay0", "paper"),
