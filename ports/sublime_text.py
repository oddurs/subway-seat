...
META = {
    ...
    "enable": {
        "where": "Preferences → Settings, after copying the file to Packages/User (Preferences → Browse Packages…)",
        "code": '"color_scheme": "{name}.sublime-color-scheme"',
        "lang": "json",
    },
    "notes": "A color scheme for Sublime Text 4 with gutter diff marks, bracket and find highlights, "
    "and hover popups in the same grounds. To follow the system appearance, set `\"color_scheme\": \"auto\"` "
    "with `dark_color_scheme` and `light_color_scheme`.",
}
...
def scheme(f):
    ...
    popup_css = (
        f"html {{ background-color: {c.mantle}; color: {c.text}; }} "
        f"a {{ color: {c.denim}; }} "
        f"code, .highlight {{ background-color: {c.crust if f.dark else c.surface0}; color: {c.green}; }} "
    ...
    rules = [rule(scope, color, styles=st) for scope, color, st in scope_rules(f)]
    rules += [
...
def build(flavors):
    ...
    for f in flavors:
        ...
        outs.append(Out(f"{f.name}.sublime-color-scheme", f"// {HEADER}\n{body}\n", flavor=f.id,
                        dest=f"Packages/User/{f.name}.sublime-color-scheme", lang="json"))
    return outs
