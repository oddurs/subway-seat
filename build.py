...
def install_table(entries: dict) -> str:
    ...
    enable  <id>  <flavor>  <config file>  <code>
    auto    <id>  <config file>  <code>
    how     <id>  <flavor|*>  <text>          # a step install.sh can only print
    """
    rows = [f"# {line}" for line in ("Generated by build.py for install.sh. Tab-separated; fields escaped for printf %b.",)]
    for e in sorted(entries.values(), key=sort_key):
        ...
        for file in e["files"]:
            ...
            if file["how"]:
                rows.append(f"how\t{pid}\t{flavor}\t{esc(file['how'])}")
        for fid, enable in (e["enable"] or {}).items():
            if enable.get("file"):
                rows.append(f"enable\t{pid}\t{fid}\t{esc(enable['file'])}\t{esc(enable['code'])}")
        if (auto := e.get("auto")) and auto.get("file"):
            rows.append(f"auto\t{pid}\t{esc(auto['file'])}\t{esc(auto['code'])}")
    return "\n".join(rows) + "\n"
...
def main() -> None:
    ...
    for path, body in outputs.items():
        write(path, body)
    if not args.only:
        for stale in sorted(x for x in DIST.iterdir() if x.name not in {d.name for d in port_dirs} | {"manifest.json", "install.tsv"}):
            if stale.is_dir():
                shutil.rmtree(stale)
